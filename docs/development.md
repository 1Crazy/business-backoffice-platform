...
- 商机列表、详情、负责人转交、阶段推进、赢单/输单收口统一复用同一套 owner-based 数据范围校验。
- 创建或更新商机时，还需要额外校验关联客户和来源线索是否处于当前用户可访问范围内，避免产生“商机可见但关联主体不可见”的断层。
- 看板中的商机指标必须与商机业务列表共享同一数据范围，但时间口径分别固定为 `createdAt`、`expectedCloseDate` 和 `closedAt`，不要混用。
...
- 销售主管仅查看本部门范围内的客户、商机、线索、提醒和看板统计，并可重新分配商机负责人。
- 销售成员仅查看本人归属数据，且无法访问超出范围的商机详情、阶段操作、线索转化和附件下载。
- 会话在执行退出后无法继续刷新；停用账号的活动会话会在下一次访问受保护接口时失效。
- 新增或重构代码必须通过 `pnpm architecture:check`，且不允许无说明地新增遗留架构例外。

商机主路径联调建议覆盖：

- 创建一个关联客户的商机，并确认默认阶段、预计金额、预计成交日期和下一步动作被正确保存。
